...
const findWatchEndpoint = (item: {
  ...
  const navEndpoint = item.navigationEndpoint as
    | { watchEndpoint?: WatchEndpoint }
    | undefined;
  const navWatch = navEndpoint?.watchEndpoint;
...
const scheduleSearch = () => {
  ...
  searchTimer = window.setTimeout(() => {
    refreshItems().catch((error) => {
      console.warn('[CommandPalette] refresh failed', error);
    });
  }, 120);
};
...
const triggerSelection = (playOnly: boolean) => {
...
const handleInputKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (items.length === 0) {
      listEl?.scrollBy({ top: 48, behavior: 'smooth' });
      return;
    }
  ...
  if (event.key === 'Enter') {
    event.preventDefault();
    const playOnly = event.shiftKey;
    Promise.resolve(triggerSelection(playOnly)).catch((error) => {
      console.warn('[CommandPalette] action failed', error);
    });
    return;
  }
...
export const onRendererLoad = ({
  ipc,
}: RendererContext<CommandPalettePluginConfig>) => {
  ipcRef = ipc;
