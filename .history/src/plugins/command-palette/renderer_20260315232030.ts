...
const findWatchEndpointFromRuns = (runs?: { navigationEndpoint?: WatchEndpoint }[]) => {
  if (!runs) return undefined;
  for (const run of runs) {
    if (run.navigationEndpoint?.videoId) return run.navigationEndpoint;
  }
  return undefined;
};
...
const findWatchEndpoint = (item: {
  ...
  const navWatch = item.navigationEndpoint;
  ...
  for (const column of item.flexColumns ?? []) {
    const runs = column.musicResponsiveListItemFlexColumnRenderer?.text
      ?.runs as { navigationEndpoint?: WatchEndpoint }[] | undefined;
...
const triggerSelection = async (playOnly: boolean) => {
  ...
  if (playOnly && playerApi?.loadVideoByPlayerVars) {
    try {
      playerApi.loadVideoByPlayerVars({
